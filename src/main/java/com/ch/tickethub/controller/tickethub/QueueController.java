...
@RestController		// 새로고침 없이 비동기 방식으로. 이 애노테이션을 붙이면 메서드가 반환하는 객체(QueueStatus)를 스프링이 알아서 JSON 형식으로 변환해서 클라이언트에게 보내줌.
...
public class QueueController {
	...
	public QueueStatus enterQueue(HttpSession session) {
		String userId = session.getId();		// << 이 부분은 추후에 로그인 및 회원기능이 전부 구현 완료되면, memberId 로 교체예정. 지금은 session id 로 진행.
