...
public class MemberServiceImpl implements MemberService {
	...
	public OauthLoginResponse loginOauthOrRegister(String oauthProvider, String oauthId, String email, String name) {
		...
		try {
			...
			if (r != 1)
				throw new RuntimeException("소셜 회원 최소가입 실패");
		...
		if (created == null)
			throw new RuntimeException("소셜 가입 후 회원 조회 실패");
...
//
//		// 필수: loginId, email, password(일반가입)
...
//		member.setRole("USER");
//		member.setGradeId(getWelcomeGradeId());
//
//		// 비밀번호 해시
//		member.setPasswordHash(PasswordUtil.hash(member.getPasswordHash()));
//
//		mailSender.send(member.getEmail(), "Tickethub 가입을 환영합니다", "<h3>가입 완료</h3>");
//
//		// 최소가입(insertMinimalMember) : phone/address 없이 profile_completed='N'
//		try {
//			int r = memberDAO.insertMinimalMember(member);
//			if (r != 1)
...
//				throw new RuntimeException("이미 가입된 이메일입니다.");
//			} else if (m.contains("uk_member_login")) {
//				throw new RuntimeException("이미 존재하는 ID가 있습니다.");
//			}
...
//	}
	...
	public LoginResult loginCheck(String loginId, String password) {
		...
		if ("N".equals(member.getProfileCompleted()))
			return LoginResult.PROFILE_REQUIRED;
	...
	@Override
	@Transactional
	public int registerFullFromDraft(com.ch.tickethub.dto.JoinDraft draft,
	    ...
	    m.setProfileCompleted("Y"); // ✅ 여기서 가입완료
