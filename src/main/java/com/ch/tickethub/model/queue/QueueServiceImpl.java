...
public class QueueServiceImpl implements QueueService{
	...
	@Override
	public void enterQueue(String userId) {		// 클라이언트가 입장하면(예매하면) 호출.
		// 1. zadd 의 Score 값에다가 timestamp 넣어서 대기열에 추가.
		redisTemplate.opsForZSet().add(WAITING_KEY, userId, System.currentTimeMillis());
	}

	@Override
	public QueueStatus getQueueStatus(String userId) {	// 나 진입할 수 있음? >> 클라이언트 화면에서 5초마다 호출할 로직.
		// 2. 먼저 이 사람이 이미 번호표 뽑고 '활동방'에 들어간 건 지 확인
		Boolean isActive = redisTemplate.opsForSet().isMember(ACTIVE_KEY, userId);
		
		if (isActive) {
			return QueueStatus.builder().userId(userId).rank(0).allowed(true).build();
		}
		...
		if (rank == null) {		// redis 는 대기열에 유저가 없다면 null 을 반환.
			return QueueStatus.builder().userId(userId).rank(-1).allowed(false).build();
		}	// 대기열에 등록되지 않는 사용자가 조회를 할 경우, redis 에 rank를 -1로 보내고 allowed 를 false로 처리.
		
		return QueueStatus.builder().userId(userId).rank(rank + 1).allowed(false).build();
	}	// 여기서 rank 에다가 +1 하는 이유는 우리는 0번째부터 시작하는 걸 알지만, 클라이언트는 0번째부터 << 라는 개념 자체가 안 익숙하므로 1을 더해준다.
